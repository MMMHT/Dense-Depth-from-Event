            ...
            for j in range(self.num_scales):
                if i == j:
                    ...
                    self.fuse_layers[-1].append(
                        nn.Sequential(nn.Conv2d(max_disp , max_disp ,
                                                kernel_size=1, bias=False),
                                      nn.BatchNorm2d(max_disp ),
                                      ))
                elif i > j:
                    layers = nn.ModuleList()
                    for k in range(i - j - 1):
                        layers.append(nn.Sequential(nn.Conv2d(max_disp , max_disp ,
...
class AdaptiveAggregation(nn.Module):
    def __init__(self, max_disp, num_scales=3, num_fusions=6,
        ...
        for i in range(self.num_scales):
            ...
            self.final_conv.append(nn.Conv2d(in_channels, max_disp , kernel_size=1))
