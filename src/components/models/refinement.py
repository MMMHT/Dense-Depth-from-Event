...
class StereoNetRefinement(nn.Module):
    ...
    def forward(self, low_disp, left_img, right_img=None):
        ...
        disp_warp = disp.floor()
        disp_warp[disp > self.in_planes-1] = self.in_planes-1
        disp_warp[disp<0] = 0


        # if torch.isnan(disp_warp) is not None:
        #     print('disp_warp is nan')
        #     disp_warp[torch.isnan(disp_warp)] = 0

        left_img_warp = torch.gather(left_img, 1, disp_warp.long())

        concat = torch.cat((disp, left_img_warp), dim=1) # [B, 2, H, W]
