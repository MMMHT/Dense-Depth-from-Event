...
class ExpLogger:
    ...
    def save_args(self, args):
        ...
        self.write(log=args_log, file_name=self._FILE_NAME['args'], mode='w', add_time=False, is_print=False)

    def save_cfg(self, cfg):
        self.write(log=str(cfg), file_name=self._FILE_NAME['config'], mode='w', add_time=False, is_print=False)
    ...
    def log_model(self, model):
        ...
        self.write(log=str(model), file_name=self._FILE_NAME['model'], mode='w', add_time=False, is_print=False)
    ...
    def log_optimizer(self, optimizer):
        self.write(log=str(optimizer), file_name=self._FILE_NAME['optimizer'], mode='w', add_time=False, is_print=False)
    ...
    def save_visualize_plt(self, image, visual_type, sequence_name, image_name ):
        visualize_root = os.path.join(self._save_root, self._DIR_NAME['visualize'], visual_type, sequence_name)
        os.makedirs(visualize_root, exist_ok=True)
        visualize_path = os.path.join(visualize_root, image_name)
        plt.imsave(visualize_path, image, cmap='plasma')
