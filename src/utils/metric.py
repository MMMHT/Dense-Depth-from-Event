...
class Metric:
    ...
    def update(self, pred, ground_truth, mask):
        ...
        for p, gt, m in zip(pred, ground_truth, mask):
            if not m.any():
                # print('Warning: no valid pixel in ground truth. Skip this image.')
                continue
        ...
        if data_count == 0:
           # self.average_meter.update(val=0.0, n=0)
...
class EndPointError(Metric):
    def calculate_error(self, pred, ground_truth, mask):
        ...
        if self.average_by == 'pixel':
            ...
            final_error = error.sum() / mask.sum()
        else:
            raise NotImplementedError
...
class DepthError(Metric):
    def __init__(self, mode = 'mean', average_by = 'image',  string_format=None):
        super().__init__(average_by='image', string_format='%6.3lf')
        self.mode = mode

    def calculate_error(self, pred, ground_truth, mask):
        pred, ground_truth = pred[mask], ground_truth[mask]
        error = torch.abs(pred - ground_truth)
        err =  error.sum() / mask.sum()
        return err
...
class RootMeanSquareErrorLog(Metric):

    def calculate_error(self, pred, ground_truth, mask):
        # pred, ground_truth, mask: (H, W)
        pred, ground_truth = pred[mask], ground_truth[mask]
        error = ((torch.log(pred) - torch.log(ground_truth)) ** 2).mean().sqrt()
...
class RelativeSquaredError(Metric):
    def calculate_error(self, pred, ground_truth, mask):
        # pred, ground_truth, mask: (H, W)
        pred, ground_truth = pred[mask], ground_truth[mask]
        error = ((pred - ground_truth) ** 2)/ (ground_truth ** 2)
...
class Accuracy(Metric):
    def __init__(self, threshold=1.25, average_by='image', string_format=None):
        super().__init__(average_by=average_by, string_format=string_format)
    ...
    def calculate_error(self, pred, ground_truth, mask):
        # pred, ground_truth, mask: (H, W)
        pred, ground_truth = pred[mask], ground_truth[mask]
        val = torch.max(pred / ground_truth, ground_truth / pred)
        acc = (val < self.threshold).float()

        if self.average_by == 'image':
            final_error = acc.sum() / val.shape[0]
        else:
            raise NotImplementedError
